...
import { selectKeybinds } from "./redux/keybindSelectors";
...
export const KeybindControl = () => {
  ...
  const moveSelectedNodes = useCallback(
    (dx: number, dy: number) => {
      selectedNodes.forEach((node: NodeData) => {
        if (node) {
          dispatch(
            updateNode({
              id: node.id,
  ...
  const handleKeyDown = useCallback(
    (event: KeyboardEvent) => {
      ...
      switch (matchingAction) {
        ...
        case KeyAction.SEND_BACKWARD:
          dispatch(sendSelectedNodesBackward());
          break;

        case KeyAction.BRING_FORWARD:
          dispatch(bringSelectedNodesForward());
          break;
      }
    },
