  ...
  const onMouseMoveResize = useCallback(
    (e: MouseEvent) => {
      ...
      if (newWidth > maxWidth) {
        newWidth = maxWidth;
        if (isWest) newEdges.left = newEdges.right - maxWidth;
        if (isEast) newEdges.right = newEdges.left + maxWidth;
      }

      // Height Limiting
      if (newHeight > maxHeight) {
        newHeight = maxHeight;
        if (isNorth) newEdges.top = newEdges.bottom - maxHeight;
        if (isSouth) newEdges.bottom = newEdges.top + maxHeight;
      }
